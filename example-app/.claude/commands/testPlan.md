...
- Focus only on one provided page/view and optionally a specific scenario as argument.
- Think very hard
- Site address is: localhost:5173
...
3. Optional: Note down into test-plan-report.md the existing overall system statements coverage (before creating new tests), and the amount of overall existing tests (for the entire system). Run the command `npm run test:browser` and extract the statements coverage and overall number of tests (not only in a specific file)
...
Important: for each proposed test, suggest how to simulate its scenario. For example, if some IDE messages are needed, or a local state - tell which and how to trigger this 8. Write One test examples in the file example-test-case.ts. Base your test file on an existing test file under /src/test/\*.test.tsx 9. Ensure the test-plan-report.md has all the sections
